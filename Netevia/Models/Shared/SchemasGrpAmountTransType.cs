...
namespace Netevia.Models.Shared
{
    ...
    /// </remarks>
    /// </summary>
    public enum SchemasGrpAmountTransType
    {
        [JsonProperty("Auth")]
        Auth,
        [JsonProperty("Sale")]
    ...
    public static class SchemasGrpAmountTransTypeExtension
    {
        public static string Value(this SchemasGrpAmountTransType value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static SchemasGrpAmountTransType ToEnum(this string value)
        {
            foreach(var field in typeof(SchemasGrpAmountTransType).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is SchemasGrpAmountTransType)
                    {
                        return (SchemasGrpAmountTransType)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum SchemasGrpAmountTransType");
        }
    }
